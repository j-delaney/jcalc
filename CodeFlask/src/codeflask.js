...
export default class CodeFlask {
  ...
  createRightSidebar () {
    this.elRightSidebar = this.createElement('div', this.elWrapper)
    this.elRightSidebar.classList.add('codeflask__right-sidebar')
  ...
  runOptions () {
    ...
    this.opts.results = this.opts.results || false
    this.opts.rightSidebar = this.opts.rightSidebar || false
    ...
    if (this.opts.rightSidebar) {
      this.elWrapper.classList.add('codeflask--has-right-sidebar')
  ...
  updateRightSidebarCount () {
    ...
    for (let i = 1; i <= this.lineCount; i++) {
      const elem = document.createElement('span')
      elem.classList.add('codeflask__right_sidebar__line')
      this.elRightSidebarLines.push(elem)
    }
    this.elRightSidebar.append(...this.elRightSidebarLines)
  }
  ...
  listenTextarea () {
    ...
    this.elTextarea.addEventListener('scroll', (e) => {
      ...
      if (this.elRightSidebar) {
        this.elRightSidebar.style.transform = `translate3d(0, -${e.target.scrollTop}px, 0)`
      }
    })
  }
  ...
  updateLines () {
    this.oldLineCount = this.lineCount || 0
    this.lines = this.code.split('\n')
    this.lineCount = this.lines.length
  }

  setLineNumber () {
    if (this.opts.lineNumbers && this.lineCount !== this.oldLineCount) {
      this.updateLineNumbersCount()
    }
  }

  setRightSidebar () {
    if (this.opts.rightSidebar && this.lineCount !== this.oldLineCount) {
      this.updateRightSidebarCount()
    }
  }
  ...
  runUpdate () {
    if (this.updateCallBack) {
      this.updateCallBack(this.lines)
    }
  }
